#!/usr/bin/env python3
"""
Ultra Fast Reverse Proxy Configuration Scanner
支持 Traefik, Ingress Nginx, HAProxy 的通用扫描器
快速且全面的反向代理配置搜索工具
"""
...
class UltraFastReverseProxyScanner:
    """Ultra Fast 反向代理配置扫描器"""
    
    def __init__(self, config_path='config.json', config_override=None, proxy_type='all'):
        if config_override:
        ...
        if 'concurrency' not in self.config:
        ...
        output_dir_name = f'reverse_proxy_scan_results_{proxy_type}'
        ...
        # Load filters based on proxy type
        self.filters, self.search_keywords = FilterManager.get_filter(proxy_type)
        self.logger.info(f"[FILTER] Loaded {len(self.filters)} filters for proxy type: {proxy_type}")
        self.logger.info(f"[FILTER] Search keywords: {', '.join(self.search_keywords[:5])}...")
        
        # Setup result files
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        self.excel_file = str(self.output_dir / f'reverse_proxy_results_{proxy_type}_{timestamp}.xlsx')
        self.csv_file = str(self.output_dir / f'reverse_proxy_results_{proxy_type}_{timestamp}.csv')
        
        # Initialize result writer
        self.result_writer = ReverseProxyResultWriter(self.csv_file, self.excel_file, proxy_type)
        
        # Statistics
        self.stats = ReverseProxySearchStats(proxy_type)
        ...
        self.logger.info(f"[OK] Ultra Fast Reverse Proxy Scanner initialized")
        self.logger.info(f"[CONFIG] Proxy type: {proxy_type}, {len(self.filters)} filters")
        ...
        self.logger.info(f"[OUTPUT] Results will be saved to CSV: {self.csv_file}")
    ...
    def _init_temp_dirs(self):
        ...
        tmp_base = Path('tmp_reverse_proxy')
        if not tmp_base.exists():
            tmp_base.mkdir(exist_ok=True)
        
        num_dirs = self.config.get('concurrency', {}).get('max_workers', 8)
        
        existing_dirs = [d for d in tmp_base.iterdir() if d.is_dir() and d.name.startswith('scan_')]
        ...
        if len(existing_dirs) >= num_dirs:
            ...
            for i in range(len(existing_dirs), num_dirs):
                temp_dir = tmp_base / f'scan_{i}_{int(time.time())}'
    ...
    def setup_logging(self):
        ...
        log_file = self.output_dir / f'reverse_proxy_scanner_{self.proxy_type}.log'
    ...
    def check_has_trusted_ips(self, content, filter_item):
        """检查是否有可信 IP 配置（根据过滤器类型）"""
        proxy_type = filter_item.get('proxy_type', 'unknown')
        
        if proxy_type == 'traefik':
            # 检查 trustedIPs
            return bool(re.search(r'trustedIPs', content, re.IGNORECASE))
        elif proxy_type == 'nginx-ingress':
            # 检查 set-real-ip-from（且不是 0.0.0.0/0）
    ...
    def scan_file_fast(self, file_path, filters):
        ...
        try:
            ...
            for filter_item in filters:
                ...
                if hit:
                    # Check if trusted IPs is configured (false positive reduction)
                    has_trusted_ips = self.check_has_trusted_ips(content, filter_item)
                    
                    # Check if this filter requires additional verification
                    requires_check = filter_item.get('requires_check')
                    if requires_check and not has_trusted_ips:
                        # This is a high-risk match (missing required security config)
                        pass
    ...
    def scan_repository_fast(self, repo_path, filters):
        ...
        # Relevant file extensions (union of all filter extensions)
        relevant_extensions = set()
        for filter_item in filters:
            relevant_extensions.update(filter_item['file_extensions'])
        # Add common extensions
        relevant_extensions.update({'.yml', '.yaml', '.toml', '.json', '.conf', '.cfg', '.haproxy'})
    ...
    def search_repositories_ultra_fast(self):
        ...
        self.logger.info(f"[SEARCH] Starting repository search for proxy type: {self.proxy_type}")
        ...
        try:
            while current_date > end_date:
                ...
                # Search for repositories in this time window using search keywords
                for keyword in self.search_keywords:
                    try:
                        current_client = self.get_best_github_client()
                        
                        # Build repository search query with time window and stars
                        if min_stars > 0:
                            repo_query = (
                                f'{keyword} '
                                f'stars:>={min_stars} '
                                f'archived:false '
                                f'created:{start_window.strftime("%Y-%m-%d")}..{end_window.strftime("%Y-%m-%d")}'
                            )
                        else:
                            repo_query = (
                                f'{keyword} '
                        ...
                        self.logger.info(f"[QUERY] Search query for '{keyword}': {repo_query}")
                        
                        repositories = current_client.search_repositories(query=repo_query)
                        total_repos = repositories.totalCount
                        self.logger.info(f"[FOUND] Found {total_repos} repositories for '{keyword}' in this time window")
                        ...
                        if total_repos > 0:
                            ...
                            for repo in repositories:
                                ...
                                # Skip if already in all_repos
                                if any(r.full_name == repo_key for r in all_repos):
                                    continue
                                ...
                                if count % 50 == 0:
                                    self.logger.info(f"[PROGRESS] Loaded {count}/{total_repos} repositories for '{keyword}'")
                                
                                # Rate limiting
                                if count % 100 == 0:
                                    time.sleep(1)
                            
                            self.logger.info(f"[COMPLETE] Processed {count} repositories for '{keyword}'")
                        
                    except Exception as search_error:
                        self.logger.error(f"[ERROR] Search failed for '{keyword}' in time range {start_window.strftime('%Y-%m-%d')} to {end_window.strftime('%Y-%m-%d')}: {search_error}")
                ...
                self.logger.info(f"[TOTAL] Found {len(unique_repos)} unique repositories in time window {start_window.strftime('%Y-%m-%d')} to {end_window.strftime('%Y-%m-%d')}")
                
                # Process repositories found in this time window
                if unique_repos:
                    self.batch_process_repositories(unique_repos)
    ...
    def run(self):
        ...
        self.logger.info(f"[START] Starting Ultra Fast Reverse Proxy Scanner for {self.proxy_type}")
        self.logger.info("=" * 50)
        ...
        try:
            ...
            self.logger.info(f"   Proxy Type: {self.proxy_type}")
...
class ReverseProxyResultWriter:
    """反向代理结果写入器"""
    
    def __init__(self, csv_file, excel_file, proxy_type):
        ...
        with open(self.csv_file, 'w', newline='', encoding='utf-8') as f:
            ...
            writer.writerow([
                ...
                'Proxy Type', 'Vulnerability Type', 'Risk Level', 'Has TrustedIPs', 'File Path'
            ])
                ...
                for filter_name, matches in matches_by_filter.items():
                    for match in matches:
                        writer.writerow([
                            ...
                            stars,
                            year,
                            match.get('proxy_type', self.proxy_type),
                            filter_name,
                            match['risk_level'],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Ultra Fast Reverse Proxy Configuration Scanner',
...
Examples:
  ...
  python ultra_fast_reverse_proxy_scanner.py --proxy-type all
    ...
    parser.add_argument(
        '--proxy-type', '-p',
        choices=['traefik', 'nginx-ingress', 'haproxy', 'all'],
        default='all',
        help='Proxy type to scan (default: all)'
    )
    ...
    try:
        ...
        # Create scanner
        scanner = UltraFastReverseProxyScanner(
            config_override=config,
            proxy_type=args.proxy_type
        )
        
        print("\n" + "="*60)
        print("REVERSE PROXY SCANNER CONFIGURATION")
        print("="*60)
        print(f"Proxy Type:   {args.proxy_type}")
        print(f"Workers:      {args.workers}")
        print(f"Output Dir:   {scanner.output_dir}")
        print(f"CSV File:     {scanner.csv_file}")
        print(f"Filters:     {len(scanner.filters)}")
        print("="*60)
        
        # Start scanning
        print(f"\n[START] Starting {args.proxy_type} configuration scan...")
        scanner.run()
